...
public class KarakterKontrolcusu : MonoBehaviour
{
    ...
    public bool KarakterKontrolEtme = true; 
    public float animasyonHizi = 0;
    float animasyonHizDegisimi = 0.03f;

    float karakterHiziZ = 0;
    public float karakterHiziHassaslik = 0.2f;
    float karakteriSolaSagaDonmeHassasligi = 0.1f;
    ...
    void Update()
    {
        if (KarakterKontrolEtme)
        {
            if (karakterYonlendirmeIzniDikey)
            {
                if(Application.platform == RuntimePlatform.Android)
                {
                    Karakteri_z_yonunde_ileriye_ayarla();                                       // 1---1
                }
            ...
            if (karakterYonlendirmeIzniYatay)
            {
                if (Application.platform == RuntimePlatform.Android)
                {
                    ...
                    {
                        KarakteriSagaGotur();                                                   // 1---4
                    }
                    else if(Input.acceleration.x < -0.1)
                    {
                        KarakteriSolaGotur();                                                   // 1---3
                    }
                }
